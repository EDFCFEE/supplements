...
def aggregate_single_tpfpfn_stats(pred_record_mat, gold_record_mat, event_role_num_list):
    """
    ...
    ], where argument 1 should support the '=' operation and empty argument is None.
    """
    ...
    for event_idx, (pred_records, gold_records) in enumerate(zip(pred_record_mat, gold_record_mat)):
        ...
        role_tpfpfn_stats = [[0] * 3 for role_idx in range(role_num)]
...
def measure_event_table_filling(pred_record_mat_list, gold_record_mat_list, event_type_roles_list, avg_type='micro',
    ...
    for pred_record_mat, gold_record_mat in zip(pred_record_mat_list, gold_record_mat_list):
        event_role_tpfpfn_stats = aggregate_single_tpfpfn_stats(
    ...
    for event_idx, role_num in enumerate(event_role_num_list):
        event_tpfpfn_stat = [0] * 3  # tp, fp, fn
        event_prf1_stat = [0] * 3
        ...
        for role_idx in range(role_num):
            ...
            for mid in range(3):
                event_tpfpfn_stat[mid] += role_tpfpfn_stat[mid]
        ...
        for mid in range(3):
            ...
            g_tpfpfn_stat[mid] += event_tpfpfn_stat[mid]
            g_prf1_stat[mid] += event_prf1_stat[mid]

        micro_event_prf1_stat = get_prec_recall_f1(*event_tpfpfn_stat)
        macro_event_prf1_stat = tuple(event_prf1_stat)
        if avg_type.lower() == 'micro':
            event_prf1_stat = micro_event_prf1_stat
        elif avg_type.lower() == 'macro':
            event_prf1_stat = macro_event_prf1_stat
        else:
        ...
        event_eval_dict = {
            ...
            'MacroPrecision': macro_event_prf1_stat[0],
            'MacroRecall': macro_event_prf1_stat[1],
            'MacroF1': macro_event_prf1_stat[2],
            'MicroPrecision': micro_event_prf1_stat[0],
            'MicroRecall': micro_event_prf1_stat[1],
            'MicroF1': micro_event_prf1_stat[2],
            'TP': event_tpfpfn_stat[0],
            'FP': event_tpfpfn_stat[1],
            'FN': event_tpfpfn_stat[2]
        }
        event_role_eval_dicts.append((event_eval_dict, role_eval_dicts))

    micro_g_prf1_stat = get_prec_recall_f1(*g_tpfpfn_stat)
    macro_g_prf1_stat = tuple(s / num_events for s in g_prf1_stat)
    if avg_type.lower() == 'micro':
        g_metric = micro_g_prf1_stat
    else:
        g_metric = macro_g_prf1_stat

    g_eval_dict = {
        'MacroPrecision': macro_g_prf1_stat[0],
        'MacroRecall': macro_g_prf1_stat[1],
        'MacroF1': macro_g_prf1_stat[2],
        'MicroPrecision': micro_g_prf1_stat[0],
        'MicroRecall': micro_g_prf1_stat[1],
        'MicroF1': micro_g_prf1_stat[2],
        'TP': g_tpfpfn_stat[0],
        'FP': g_tpfpfn_stat[1],
        'FN': g_tpfpfn_stat[2]
    }
